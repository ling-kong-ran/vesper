...
- **消息**：agent 消息卡片化（solid 卡 + 描边），左侧使用 32px 原创“软糯星星精灵”SVG 状态头像（暖黄 / 珊瑚粉 / 淡紫渐变、深色小眼睛、淡腮红和极小表情）；用户气泡见 §3.3
- **状态/流式**：等待输入时精灵轻微软弹呼吸、眨眼和星光闪烁；思考时身体左右轻摆、眼睛搜索、腮红/小嘴变化、星星跳动；鼠标靠近时眼睛轻微跟随，替代原三点 typing-dot
