...
test('model configuration exposes built-in Kimi and GLM providers', async (t) => {
  ...
  const codex = config.providers.find((provider) => provider.id === 'openai-codex')
  ...
  assert.equal(codex.custom, false)
  assert.ok(codex.models.some((model) => model.id.includes('codex')))
