...
export class LocalMemoryRuntime {
  ...
  listSpaces() {
    const rows = this.requireDb().prepare(`
      SELECT spaces.*, COUNT(memories.id) AS node_count
  ...
  listMemories({ spaceId, query = '', limit = 100 } = {}) {
    ...
    if (!query.trim()) {
      return db.prepare('SELECT * FROM memories WHERE space_id = ? ORDER BY importance DESC, updated_at DESC LIMIT ?')
  ...
  listLinks(spaceId) {
    return this.requireDb().prepare('SELECT id, source_id, target_id, relation, weight, created_at FROM memory_links WHERE space_id = ? ORDER BY weight DESC')
      .all(spaceId).map((row) => ({
        id: row.id,
        sourceId: row.source_id,
  ...
  refreshRelatedLinks(id, spaceId, vector) {
    ...
    const related = db.prepare('SELECT id, embedding FROM memories WHERE space_id = ? AND id <> ?').all(spaceId, id)
  ...
  remember(input = {}) {
    ...
    const existingRows = db.prepare('SELECT * FROM memories WHERE space_id = ?').all(spaceId)
    ...
    if (duplicate && input.dedupe !== false) {
      db.prepare(`
        UPDATE memories SET title = ?, content = ?, type = ?, source_type = ?, source_id = ?, source_path = ?,
          session_id = ?, cwd = ?, importance = ?, embedding = ?, updated_at = ? WHERE id = ?
      `).run(
        ...
        Math.max(importance, Number(duplicate.importance || 0)), embeddingBuffer(vector), now, duplicate.id,
      )
    ...
    db.prepare(`
      INSERT INTO memories (id, space_id, title, content, type, source_type, source_id, source_path, session_id, cwd,
        importance, embedding, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      ...
      embeddingBuffer(vector), now, now,
    )
  ...
  updateMemory(id, input = {}) {
    ...
    const vector = localEmbedding(`${title}\n${content}`)
    db.prepare(`
      UPDATE memories SET space_id = ?, title = ?, content = ?, type = ?, source_path = ?, importance = ?, embedding = ?, updated_at = ? WHERE id = ?
    `).run(nextSpaceId, title, content, type, cleanText(input.sourcePath ?? current.sourcePath, 1000), importance, embeddingBuffer(vector), now, id)
  ...
  search(query, { cwd = '', spaceIds = null, limit = 6, minScore = 0.12, trackAccess = true } = {}) {
    ...
    const db = this.requireDb()
    const rows = db.prepare(`SELECT * FROM memories WHERE space_id IN (${placeholders})`).all(...ids)
    ...
    if (ftsQuery) {
      try {
        const matches = db.prepare(`
          ...
          WHERE memory_fts MATCH ? AND memories.space_id IN (${placeholders}) LIMIT 100
        `).all(ftsQuery, ...ids)
    ...
    const ranked = rows.map((row) => {
      ...
      const score = relevance * 0.85 + Number(row.importance || 0.5) * 0.1 + recency * 0.05
