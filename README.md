...
渠道页只提供能够与 Agent 双向通信的平台连接。目前支持飞书应用机器人：调用飞书官方 Node SDK 的一键注册能力，用户扫码确认后自动创建机器人应用、保存凭据，并通过 WebSocket 长连接接收私聊或群聊 @ 消息。无需手工填写 App ID、App Secret，也不需要公网 IP、域名或回调地址。

每个飞书聊天会映射到独立的 Pi Coder 会话，支持文字、图片和文档输入，Agent 生成的图片、视频和文件也会回传飞书。默认仅允许扫码创建者使用；可在渠道页调整租户访问范围和新会话工作目录。企业微信在没有本地双向长连接方案前不作为可连接渠道展示。

## 用户数据

Provider、模型、工具权限、会话、资产和渠道默认保存在用户目录下的 Pi Agent 配置目录中；可通过 `PI_CODING_AGENT_DIR` 覆盖。飞书应用凭据和会话映射位于 `pi-coder-channels.json`，接口只返回脱敏信息。不要把 API Key 或应用密钥写入仓库文件。
