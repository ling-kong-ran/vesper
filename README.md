...
渠道页支持飞书自定义群机器人和企业微信群消息推送。两者都采用官方 Webhook 流程，支持连接测试、飞书签名校验、启停以及按“会话完成 / 执行失败”自动通知。Webhook 仅允许对应平台的官方 HTTPS 域名，接口响应只返回脱敏地址。

飞书自定义机器人和企业微信消息推送都需要在目标群内创建，适用于单向通知；它们不能接收群成员发给 Agent 的消息。双向对话需要单独接入平台应用机器人及事件订阅。

## 用户数据

Provider、模型、工具权限、会话、资产和渠道默认保存在用户目录下的 Pi Agent 配置目录中；可通过 `PI_CODING_AGENT_DIR` 覆盖。渠道配置位于 `pi-coder-channels.json`。不要把 API Key 或 Webhook 写入仓库文件。
