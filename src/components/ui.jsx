...
export function Toggle({ defaultOn = false, value, onChange, disabled = false }) {
  ...
  return <button type="button" aria-pressed={on} disabled={disabled} className={`toggle relative inline-flex h-8 w-12 min-w-12 items-center rounded-full border-0 p-1 transition disabled:cursor-not-allowed disabled:opacity-45 ${on ? 'on bg-[var(--blue)]' : 'bg-[var(--control-muted)]'}`} onClick={(event) => { event.stopPropagation(); if (onChange) onChange(!on); else setInternal(!on) }}><i className={`block size-6 rounded-full bg-white shadow-sm transition ${on ? 'translate-x-4' : ''}`} /></button>
}
