...
const TOAST_TONES = {
  ...
  info: {
    Icon: Info,
    classes: 'border-[var(--accent-border)] bg-[var(--accent-soft)] text-[var(--accent-strong)]',
  },
}
...
export function Toggle({ defaultOn = false, value, onChange, disabled = false }) {
  ...
  return <button type="button" aria-pressed={on} disabled={disabled} className={`toggle relative inline-flex h-8 w-12 min-w-12 items-center rounded-full border-0 p-1 transition disabled:cursor-not-allowed disabled:opacity-45 ${on ? 'on bg-[var(--blue)]' : 'bg-[var(--control-muted)]'}`} onClick={(event) => { event.stopPropagation(); if (onChange) onChange(!on); else setInternal(!on) }}><i className={`block size-6 rounded-full bg-[var(--control-thumb)] shadow-sm transition ${on ? 'translate-x-4' : ''}`} /></button>
}
...
export function AppDialog({ dialog, onClose, onFinish }) {
  ...
  return <div className="fixed inset-0 z-70 grid place-items-center bg-[var(--modal-overlay)] p-5 backdrop-blur-[3px]" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><form role="dialog" aria-modal="true" aria-labelledby="app-dialog-title" className="w-full max-w-[430px] rounded-[13px] border border-[var(--surface-highlight)] bg-[var(--solid)] p-[18px] text-[var(--text)] shadow-[0_26px_70px_-25px_var(--shadow-strong)]" onSubmit={submit}><div className="flex items-start justify-between gap-3"><div><h2 id="app-dialog-title" className="text-[17px] leading-6 font-bold">{dialog.title}</h2>{dialog.message && <p className="mt-1.5 text-[13px] leading-5 text-[var(--muted)]">{dialog.message}</p>}</div><button type="button" aria-label="关闭对话框" className="grid size-8 shrink-0 place-items-center rounded-lg border-0 bg-transparent text-[var(--muted)] hover:bg-[var(--surface-muted)]" onClick={onClose}><X size={18} /></button></div>{dialog.type === 'prompt' && <label className="mt-4 block text-[12px] font-semibold text-[var(--text-soft)]">{dialog.inputLabel || '名称'}<input ref={inputRef} className="mt-2 h-10 w-full rounded-lg border border-[var(--stroke)] bg-[var(--solid)] px-3 text-[13px] outline-none focus:border-[var(--accent-border)] focus:ring-3 focus:ring-[var(--accent-ring)]" value={value} maxLength={dialog.maxLength || 120} onChange={(event) => setValue(event.target.value)} /></label>}<div className="mt-[18px] flex justify-end gap-2"><button type="button" className="inline-flex min-h-9 items-center justify-center rounded-lg border border-[var(--stroke)] bg-[var(--surface-subtle)] px-3 text-[12px] font-semibold" onClick={onClose}>取消</button><button type="submit" disabled={dialog.type === 'prompt' && !value.trim()} className={`inline-flex min-h-9 items-center justify-center rounded-lg border px-3 text-[12px] font-semibold text-[var(--on-accent)] disabled:cursor-not-allowed disabled:opacity-50 ${dialog.tone === 'danger' ? 'border-[var(--danger)] bg-[var(--danger)]' : 'border-[var(--blue)] bg-[var(--blue)]'}`}>{dialog.confirmLabel}</button></div></form></div>
}
