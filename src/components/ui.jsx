...
export function PreviewNotice({ children }) {
  const { t } = useI18n()
  const content = typeof children === 'string' ? t(children) : children || t('演示界面 · 尚未连接真实运行时')
  return <div className="mb-3 flex min-h-10 items-center gap-2 rounded-lg border border-[var(--warning-border)] bg-[var(--warning-soft)] px-3 text-[12px] font-semibold text-[var(--warning-strong)]"><Info size={16} className="shrink-0" />{content}</div>
}
