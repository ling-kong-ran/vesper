...
export function ChannelsPage({ notify, registerPrimaryAction, requestConfirm }) {
  ...
  const resetScope = async (scope) => {
    const approved = await requestConfirm({ title: '重置渠道会话', message: `重置“${scope.title}”绑定的 ${APP_NAME} 会话？`, confirmLabel: '重置' })
    if (!approved) return
