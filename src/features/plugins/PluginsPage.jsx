...
import { toolNameKey } from './tool-labels.js'
...
export function PluginsPage({ query, notify, registerPrimaryAction, onStatusChange }) {
  ...
  const filtered = draft.filter((tool) => {
    ...
    return `${t(toolNameKey(tool))} ${tool.id} ${t(tool.description)}`.toLowerCase().includes(query.toLowerCase())
  })
