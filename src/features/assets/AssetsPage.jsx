...
export function AssetsPage({ query, notify, registerPrimaryAction, onUse, requestConfirm }) {
  ...
  return <div className="asset-page">
    ...
    <div className="asset-summary"><span><strong>{assets.length}</strong> {t('个资产')}</span><span>{t('对话附件与 Agent 产物，会在这里自动汇聚')}</span></div>
    ...
    })}</div> : <Panel className="empty-state"><StarOrbit size={46} /><h2>{t('这里还没有留下产物')}</h2><p>{t('把链接带来，或在对话中加入附件；Vesper 会把沿途生成的文件一并收好。')}</p><button className="button primary" onClick={() => setLinkModal(true)}><Link2 size={14} />{t('添加链接')}</button></Panel>}
