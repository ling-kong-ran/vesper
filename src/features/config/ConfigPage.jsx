    ...
    <div className="split-list-detail config-layout">
      ...
      <div className="detail-stack">
        ...
        <div className="config-bottom">
          ...
          <Panel className="usage-card"><SectionTitle title="运行时状态" /><div className="usage-number"><span>Engine</span><strong>{APP_NAME} Runtime</strong></div><div className="usage-number"><span>Provider</span><strong>{selectedProvider.name}</strong></div><div className="usage-number"><span>Models</span><strong>{selectedProvider.models.length}</strong></div><div className="usage-number"><span>状态</span><strong>{selectedProvider.enabled ? '启用' : '停用'}</strong></div>{error && <div className="config-error"><AlertTriangle size={13} />{error}</div>}<button className="button primary wide" disabled={saving || !draft.model || !selectedProvider.enabled} onClick={save}>{saving ? <RefreshCw className="spin" size={14} /> : <Save size={14} />}{saving ? '保存中…' : selectedProvider.enabled ? '保存并设为默认' : '启用后可保存'}</button></Panel>
        </div>
      </div>
    </div>
