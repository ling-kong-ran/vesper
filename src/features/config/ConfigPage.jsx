...
export function ConfigPage({ notify, registerPrimaryAction, section, setSection, onBrowserNotificationChange, requestConfirm, update }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      notify(t(provider.models.some((model) => model.kind === 'chat') ? 'Agent 配置已保存，新会话将使用该模型' : '视觉模型配置已保存'))
    } catch (caught) {
      setError(caught.message)
    } finally {
      setSaving(false)
    ...
    <div className="split-list-detail config-layout">
      ...
      <div className="detail-stack">
        <Panel>
          ...
          {chatModels.length > 0 ? <><label className="field-label">{t('默认对话模型')}<span className="select-wrap"><select value={draft.model} onChange={(event) => selectModel(event.target.value)}>{chatModels.map((model) => <option key={model.id} value={model.id}>{model.name}</option>)}</select><ChevronDown size={13} /></span></label>{!codexOAuth && <label className="field-label">{t('模型 Base URL')}<input value={draft.modelBaseUrl} onChange={(event) => setDraft({ ...draft, modelBaseUrl: event.target.value })} placeholder={t('可选；为当前模型覆盖 Provider Base URL')} /></label>}<div className="tag-field"><Badge>{draft.provider}</Badge><Badge>{t(selectedModel?.reasoning ? '支持推理' : '标准模型')}</Badge><Badge tone="gray">{selectedModel?.contextWindow ? `${Math.round(selectedModel.contextWindow / 1000)}K context` : t('自动上下文')}</Badge>{selectedModel?.baseUrlOverride && <Badge tone="amber">{t('独立 Base URL')}</Badge>}</div></> : <div className="permission-note"><Sparkles size={16} /><span><strong>{t('纯视觉模型 Provider')}</strong><small>{t('仅用于生图或生视频，不参与 Agent 对话模型选择。')}</small></span></div>}
...
function ProviderModelModal({ provider, connectionDraft, autoDiscover, onClose, onSynchronized, onCreated }) {
  ...
  const visibleCatalog = catalog.filter((model) => !normalizedSearch || model.id.toLowerCase().includes(normalizedSearch) || model.name.toLowerCase().includes(normalizedSearch))
